...
- `mypusht-smoke-env` 会打印环境名、步数、success 和 reward。
- `--help` 命令能正常打印 CLI 参数。
...
```bash
mypusht-convert --raw_dir outputs/data/raw_episodes --out_dir outputs/data/lerobot_dataset --fps 10
```
...
| 策略             | 数据使用方式                                                           | 采集时需要注意                                                                 |
| ---------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| BC-MLP           | 主要使用低维状态，例如机器人状态、mocap_xy、object_pose 和 goal_pose。 | 轨迹要覆盖不同初始位置和目标误差，否则低维状态到动作的映射容易只记住局部行为。 |
...
BC-MLP 是最基础的行为克隆策略。它把低维观测拼成一个向量，例如机器人关节状态、mocap_xy、object_pose 和 goal_pose，然后用多层感知机直接回归当前步的 delta action。
...
mypusht-train bc-cnn --dataset outputs/data/lerobot_dataset --cache-dir outputs/cache/bc_cnn
mypusht-train act --dataset outputs/data/lerobot_dataset --cache-dir outputs/cache/act
mypusht-train diffusion-policy --dataset outputs/data/lerobot_dataset --cache-dir outputs/cache/dp
```
...
| ---------------- | -------: | --------------: | ------------------------------------ |
| BC-MLP           |   `3000` |           `128` | `outputs/models/bc_mlp.pt`           |
| BC-CNN           |  `10000` |            `64` | `outputs/models/bc_cnn.pt`           |
| ACT              |   `8000` |            `32` | `outputs/models/act.pt`              |
...
```bash
python -m mypusht.data.sequence_dataset --dataset outputs/data/lerobot_dataset --max-items 200
```
...
```bash
mypusht-eval --policy bc_mlp --ckpt outputs/models/bc_mlp.pt --split normal --episodes 10 --no-display
mypusht-eval --policy bc_cnn --ckpt outputs/models/bc_cnn.pt --split wide --episodes 10 --no-display
...
```bash
mypusht-eval --policy heuristic --split normal --episodes 3 --save-video --no-display
```
...
| 命令                | Python 入口                         | 用途                          |
| ------------------- | ----------------------------------- | ----------------------------- |
| `mypusht-smoke-env` | `mypusht.envs.smoke:main`           | 环境最小运行检查              |
